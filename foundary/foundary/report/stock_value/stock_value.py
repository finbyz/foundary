...
class StockBalanceReport:
	...
	def set_financial_year_dates(self):
		"""
		Determine financial year start and end dates
		Assumes financial year starts in April and ends in March
		"""
		if self.to_date.month < 4:
			self.fy_start = datetime(self.to_date.year - 1, 4, 1).date()
			self.fy_end = datetime(self.to_date.year, 3, 31).date()
		else:
			self.fy_start = datetime(self.to_date.year, 4, 1).date()
			self.fy_end = datetime(self.to_date.year + 1, 3, 31).date()
	...
	def run(self):
		...
		return self.columns, self.data,None,chart_data
	...
	def get_chart_data(self):
		...
		"""
		Generate chart data showing stock value across months in the financial year
		"""
		# Initialize monthly stock value dictionary
		monthly_stock_value = {}
		month_labels = []
		...
		while current_date <= self.fy_end:
			...
			monthly_stock_value[month_name] = 0
			current_date = (current_date + timedelta(days=32)).replace(day=1)

		# Calculate stock value for each month
		for row in self.data:
			# Use the end date for the row's month
			row_date = self.to_date
			month_name = row_date.strftime("%B %Y")
			
			# Sum stock value for all items/warehouses
			monthly_stock_value[month_name] += row.get('bal_val', 0)
		...
		chart = {
			"data": {
				...
				"datasets": [
					{
						"name": _("Monthly Stock Value"),
						"values": [monthly_stock_value.get(month, 0) for month in month_labels]
					}
				]
			},
			"type": "bar",
			"height": 300,
			"colors": ["#7cd6fd"],
			"title": _("Monthly Stock Value ({0} Financial Year)".format(self.fy_start.year)),
			"x_axis_title": _("Months"),
			"y_axis_title": _("Stock Value ({0})".format(self.company_currency))
		}
